...
public:
  ...
  ///
  /// This is lazily computed.  The lines are owned by the SrcMgr
  ...
  getBufferOrNone(DiagnosticEngine &Diag, FileManager &FM,
...
public:
  ...
  static FileInfo get(SrcLoc IL, ContentCache &Con,
  ...
  SrcLoc getIncludeLoc() const { return IncludeLoc; }
...
class ExpansionInfo {
  ...
  /// invalid location.
  SrcLoc ExpansionLocStart, ExpansionLocEnd;
...
public:
  ...
  SrcLoc getExpansionLocStart() const { return ExpansionLocStart; }
...
class SLocEntry {
  static constexpr int OffsetBits = 8 * sizeof(SrcLoc::UIntTy) - 1;
  SrcLoc::UIntTy Offset : OffsetBits;
  LLVM_PREFERRED_TYPE(bool)
  SrcLoc::UIntTy IsExpansion : 1;
  union {
    FileInfo File;
...
public:
  ...
  SrcLoc::UIntTy getOffset() const { return Offset; }
  ...
  static SLocEntry getOffsetOnly(SrcLoc::UIntTy Offset) {
  ...
  static SLocEntry get(SrcLoc::UIntTy Offset, const FileInfo &FI) {
    assert(!(Offset & (1ULL << OffsetBits)) && "Offset is too large");
    SLocEntry E;
    E.Offset = Offset;
    E.IsExpansion = false;
  ...
  static SLocEntry get(SrcLoc::UIntTy Offset, const ExpansionInfo &Expansion) {
    assert(!(Offset & (1ULL << OffsetBits)) && "Offset is too large");
    SLocEntry E;
    E.Offset = Offset;
    E.IsExpansion = true;
...
public:
  ...
  /// Get the index ID for the loaded SrcLoc offset.
  ///
  /// \returns Invalid index ID (0) if an error occurred that prevented the
  /// SLocEntry  from being loaded.
  virtual int getSLocEntryID(SrcLoc::UIntTy SLocOffset) = 0;
...
class SrcMgr final : public RefCountedBase<SrcMgr> {
  ...
  llvm::DenseMap<FileEntryRef, sm::ContentCache *> FileInfos;
  ...
  std::vector<sm::ContentCache *> MemBufferInfos;

  /// The table of SLocEntries that are local to this module.
  ///
  /// Positive FileIDs are indexes into this table. Entry 0 indicates an invalid
  /// expansion.
  SmallVector<sm::SLocEntry, 0> LocalSLocEntryTable;

  /// The table of SLocEntries that are loaded from other modules.
  ///
  /// Negative FileIDs are indexes into this table. To get from ID to an index,
  /// use (-ID - 2).
  llvm::PagedVector<sm::SLocEntry> LoadedSLocEntryTable;
  ...
  mutable const sm::ContentCache *LastLineNoContentCache;
  ...
  mutable std::unique_ptr<sm::ContentCache> FakeContentCacheForRecovery;

  mutable std::unique_ptr<sm::SLocEntry> FakeSLocEntryForRecovery;
...
public:
  ...
  DiagnosticEngine &getDiagnostics() const { return Diag; }
  ...
  FileID createFileID(FileEntryRef SourceFile, SrcLoc IncludePos,
                      sm::CharacteristicKind FileCharacter, int LoadedID = 0,
                      SrcLoc::UIntTy LoadedOffset = 0);
  ...
  FileID createFileID(std::unique_ptr<llvm::MemoryBuffer> Buffer,
                      sm::CharacteristicKind FileCharacter = sm::C_User,
                      int LoadedID = 0, SrcLoc::UIntTy LoadedOffset = 0,
  ...
  SrcLoc createMacroArgExpansionLoc(SrcLoc SpellingLoc, SrcLoc ExpansionLoc,
                                    unsigned Length);
  ...
  SrcLoc createExpansionLoc(SrcLoc SpellingLoc, SrcLoc ExpansionLocStart,
  ...
  /// std::nullopt.
  std::optional<llvm::MemoryBufferRef>
  getBufferOrNone(FileID FID, SrcLoc Loc = SrcLoc()) const {
  ...
  llvm::MemoryBufferRef getBufferOrFake(FileID FID,
                                        SrcLoc Loc = SrcLoc()) const {
  ...
  const FileEntry *
  getFileEntryForSLocEntry(const sm::SLocEntry &SLocEntry) const {
  ...
  void setNumCreatedFIDsForFileID(FileID FID, unsigned NumFIDs,
    ...
    const_cast<sm::FileInfo &>(Entry->getFile()).NumCreatedFIDs = NumFIDs;
  }
  ...
  SrcLoc getLocForStartOfFile(FileID FID) const {
    if (auto *Entry = getSLocEntryForFile(FID))
      return SrcLoc::getFileLoc(Entry->getOffset());
    return SrcLoc();
  }
  ...
  SrcLoc getLocForEndOfFile(FileID FID) const {
    if (auto *Entry = getSLocEntryForFile(FID))
      return SrcLoc::getFileLoc(Entry->getOffset() + getFileIDSize(FID));
    return SrcLoc();
  }
  ...
  SrcLoc getComposedLoc(FileID FID, unsigned Offset) const {
    ...
    SrcLoc::UIntTy GlobalOffset = Entry->getOffset() + Offset;
  ...
  bool isInSLocAddrSpace(SrcLoc Loc, SrcLoc Start, unsigned Length,
                         SrcLoc::UIntTy *RelativeOffset = nullptr) const {
    assert(((Start.getOffset() < NextLocalOffset &&
             Start.getOffset() + Length <= NextLocalOffset) ||
            (Start.getOffset() >= CurrentLoadedOffset &&
             Start.getOffset() + Length < MaxLoadedOffset)) &&
           "Chunk is not valid SLoc address space");
    SrcLoc::UIntTy LocOffs = Loc.getOffset();
    SrcLoc::UIntTy BeginOffs = Start.getOffset();
    SrcLoc::UIntTy EndOffs = BeginOffs + Length;
  ...
  bool isInSameSLocAddrSpace(SrcLoc LHS, SrcLoc RHS,
                             SrcLoc::IntTy *RelativeOffset) const {
    SrcLoc::UIntTy LHSOffs = LHS.getOffset(), RHSOffs = RHS.getOffset();
  ...
  sm::CharacteristicKind getFileCharacteristic(SrcLoc Loc) const;
  ...
  bool isInExternCSystemHeader(SrcLoc Loc) const {
    return getFileCharacteristic(Loc) == sm::C_ExternCSystem;
  }
  ...
  bool isInFileID(SrcLoc Loc, FileID FID,
    ...
    SrcLoc::UIntTy Offs = Loc.getOffset();
  ...
  SrcLoc translateFileLineCol(const FileEntry *SourceFile, unsigned Line,
                              unsigned Col) const;
  ...
  bool isBeforeInSLocAddrSpace(SrcLoc LHS, SrcLoc::UIntTy RHS) const {
    SrcLoc::UIntTy LHSOffset = LHS.getOffset();
  ...
  using fileinfo_iterator =
      llvm::DenseMap<FileEntryRef, sm::ContentCache *>::const_iterator;
  ...
  void noteSLocAddressSpaceUsage(DiagnosticEngine &Diag,
  ...
  const sm::SLocEntry &getLocalSLocEntry(unsigned Index) const {
  ...
  const sm::SLocEntry &getLoadedSLocEntry(unsigned Index,
                                          bool *Invalid = nullptr) const {
  ...
  const sm::SLocEntry &getSLocEntry(FileID FID, bool *Invalid = nullptr) const {
  ...
  SrcLoc::UIntTy getNextLocalOffset() const { return NextLocalOffset; }
  ...
  AllocateLoadedSLocEntries(unsigned NumSLocEntries, SrcLoc::UIntTy TotalSize);
...
private:
  ...
  sm::ContentCache &getFakeContentCacheForRecovery() const;

  const sm::SLocEntry &loadSLocEntry(unsigned Index, bool *Invalid) const;

  const sm::SLocEntry *getSLocEntryOrNull(FileID FID) const {
    bool Invalid = false;
    const sm::SLocEntry &Entry = getSLocEntry(FID, &Invalid);
    return Invalid ? nullptr : &Entry;
  }

  const sm::SLocEntry *getSLocEntryForFile(FileID FID) const {
  ...
  const sm::SLocEntry &getSLocEntryByID(int ID, bool *Invalid = nullptr) const {
  ...
  const sm::SLocEntry &getLoadedSLocEntryByID(int ID,
                                              bool *Invalid = nullptr) const {
    return getLoadedSLocEntry(static_cast<unsigned>(-ID - 2), Invalid);
  }

  FileID getFileID(SrcLoc::UIntTy SLocOffset) const {
  ...
  bool isLocalOffset(SrcLoc::UIntTy SLocOffset) const {
    return SLocOffset < CurrentLoadedOffset;
  }

  bool isLoadedOffset(SrcLoc::UIntTy SLocOffset) const {
    return SLocOffset >= CurrentLoadedOffset;
  }

  /// Implements the common elements of storing an expansion info struct into
  /// the SLocEntry table and producing a source location that refers to it.
  SrcLoc createExpansionLocImpl(const sm::ExpansionInfo &Expansion,
  ...
  inline bool isOffsetInFileID(FileID FID, SrcLoc::UIntTy SLocOffset) const {
    const sm::SLocEntry &Entry = getSLocEntry(FID);
  ...
  FileID createFileIDImpl(sm::ContentCache &File, StringRef Filename,
                          SrcLoc IncludePos,
                          sm::CharacteristicKind DirCharacter, int LoadedID,
                          SrcLoc::UIntTy LoadedOffset);

  sm::ContentCache &getOrCreateContentCache(FileEntryRef SourceFile,
  ...
  FileID getFileIDSlow(SrcLoc::UIntTy SLocOffset) const;
  FileID getFileIDLocal(SrcLoc::UIntTy SLocOffset) const;
  FileID getFileIDLoaded(SrcLoc::UIntTy SLocOffset) const;
  ...
  getDecomposedExpansionLocSlowCase(const sm::SLocEntry *E) const;
  std::pair<FileID, unsigned>
  getDecomposedSpellingLocSlowCase(const sm::SLocEntry *E,
...
/// SrcMgr and necessary dependencies (e.g. VFS, FileManager) for a
/// single in-memorty file.
class SrcMgrForFile {
public:
  /// Creates SrcMgr and necessary dependencies (e.g. VFS, FileManager).
